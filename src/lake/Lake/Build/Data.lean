...
-/
public opaque DataType (kind : Name) : Type u

/-- A `Name` descriptor of a data type. -/
public class DataKind (α : Type u) where
...
-/
public class OptDataKind (α : Type u) where
...
public instance : Inhabited (OptDataKind α) := ⟨OptDataKind.anonymous⟩

@[inline] public def isAnonymous (self : OptDataKind α) : Bool :=
  self.name.isAnonymous

public theorem isAnonymous_iff_name_isAnonymous : isAnonymous k ↔ k.name.isAnonymous := by
  simp [isAnonymous]

public theorem eq_data_type
  {self : OptDataKind α} (h : ¬ self.isAnonymous) : α = DataType self.name
:= self.wf h

public instance [DataKind α] : OptDataKind α where
  name := DataKind.name α
  wf _ := DataKind.wf.2

public instance : CoeOut (OptDataKind α) Lean.Name := ⟨(·.name)⟩
...
-/
public opaque FacetOut (facet : Name) : Type
...
as needed (via `facet_data`).
-/
public abbrev FacetData (kind facet : Name) := FacetOut (kind ++ facet)

public instance [h : FamilyDef FacetOut (kind ++ facet) α] : FamilyDef (FacetData kind) facet α :=
  ⟨h.fam_eq⟩

public instance [h :  FamilyDef (FacetData kind) facet α] : FamilyDef FacetOut (kind ++ facet) α :=
  ⟨h.fam_eq⟩
...
-/
public abbrev ModuleData := FacetData Module.facetKind
...
-/
public abbrev PackageData := FacetData Package.facetKind
...
-/
public abbrev LibraryData := FacetData LeanLib.facetKind

@[inherit_doc LibraryData]
public abbrev LeanLibData := LibraryData
...
-/
public opaque CustomOut (target : Name × Name) : Type
...
as needed (via `custom_data`).
-/
public abbrev CustomData (package target : Name) := CustomOut (package, target)

public instance [h : FamilyDef CustomOut (p, t) α] : FamilyDef (CustomData p) t α :=
  ⟨h.fam_eq⟩
...
public instance [FamilyOut DataType Package.facetKind α]
